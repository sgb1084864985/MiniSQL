...
bool BufferManager::ForcedReadPageFromFile(Page& page)
{
	...
	if (!fp) {
		cout << "read failed in file " << filename.c_str() << endl;
		exit(0);
	}
	if (fp) 
	{
...
bool BufferManager::ForcedWritePageToFile(Page& page)
{
	...
	FILE* fp = fopen(filename.c_str(), "rb+");
	if (!fp) {
		cout << "write has failed in file " + page.tableName + ".record" << endl;
	}
...
void BufferManager::LRUCounterAddExceptCurrent(int index)//计数器函数，每当一个区块被使用时，缓存区内其他区块的计数器会进行自增，被使用的置0
{
	for (int i = 0; i < CACHE_CAPACITY; i++) {
		LRUCounter[i]++;
		if (LRUCounter[i] < 0) {
			cout << "overflow in LRUCounter" << endl;
			exit(0);
		}
	}
	LRUCounter[index] = 0;
